...
healthrestore = pygame.image.load("healthonpodloga.png").convert_alpha()
...
# boss_bullets_list = pygame.sprite.Group()
# def boss_spawning_bullets(x,y):
    # boss_strzal1 = projectile(x + 50,y + 50)
    # boss_strzal2 = projectile(x + 50,y + 50)
    # boss_strzal3 = projectile(x + 50,y + 50)
    # boss_bullets_list.add(boss_strzal1)
    #boss_bullets_list.add(boss_strzal2)
    # boss_bullets_list.add(boss_strzal3)
    # boss_strzal1.boss_shooting1(x,y)
    #boss_strzal2.boss_shooting2(x,y)
    # boss_strzal3.boss_shooting3(x,y)
...
def main():
    ...
    while running:
        ...
        if level%3 == 0 and mapnumber != "1":
            # boss_spawning_bullets(boss1.rect.x, boss1.rect.y)
            print(boss1.rect.x, boss1.rect.y)
        ...
        if (level%3 == 0 and mapnumber != "1"):
            ...
            # boss_bullets_list.draw(world)
            # boss_bullets_list.update()
        ...
        if money >= 10 and mapnumber == "1": 
            world.blit(attackspeeditem, (20, 20))
            if player.rect.x >= 20 and player.rect.x <= 120 and player.rect.y >= 20 and player.rect.y <= 120:
                attackspeed -= 400
                money -= 10

        #shop cross shooting
        if money >= 15 and mapnumber == "1" and crossshooter == False:
            world.blit(crossshooting, (10, 750))
            if player.rect.x >= 10 and player.rect.x <= 40 and player.rect.y >= 750 and player.rect.y <= 950:
                money -=15
                crossshooter = True
        #shop life restore
        if money >= 5 and mapnumber == "1":   
            world.blit(healthrestore, (850, 850))
            if player.rect.x >= 850 and player.rect.x <= 950 and player.rect.y >= 850 and player.rect.y <= 950:
                if lifes == 4:
                    money -= 5
                    lifes += 1
                    hp5.swap_back()
